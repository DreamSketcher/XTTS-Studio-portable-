...
│       │
│       │   ── отдельные окна ──
│       ├── chat_window.py        ← точка входа окна AI-чата (делегирует в chat_window/)
...
> ℹ️ `engine/gui/chat_window.py` и `engine/gui/chat_window/` сосуществуют в переходный период: старый файл — точка входа для обратной совместимости, вся новая логика окна чата уже вынесена в пакет `chat_window/`.
