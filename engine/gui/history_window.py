...
Редизайн (в стиле output_window.py): скруглённые карточки записей, единая
"таблетка" с кнопками сверху, современный скроллируемый список
(CTkScrollableFrame), плавающая карточка мини-плеера с waveform (название/
время наложены прямо на волну), круглые кнопки управления, круглый
всплывающий регулятор громкости. Активная запись подсвечивается только
рамкой (border), а не сменой фона — это устраняет визуальный баг с
несовпадающим фоном дочерних виджетов ("чёрный контур") в любой теме.

Позволяет прослушать результат конкретной генерации прямо из истории, без
копирования файлов (воспроизведение идёт напрямую из уже существующего
output-файла в outputs/, никакого лишнего мусора в проекте не создаётся).
"""
import json
import os
import threading
...
def _safe_t(key, fallback):
    """t() с безопасным откатом на fallback, если ключ локализации
    ещё не добавлен в i18n.py."""
    try:
        val = t(key)
...
def open_history():
    ...
    def _round_btn(parent, text, cmd, diameter=36, primary=False, danger=False, disabled=False):
        ...
        btn = CompatCTkButton(
            ...
            font=("Segoe UI", scaled_font_size(14 if primary else 12)),
        )
        if disabled:
    ...
    # ══════════════════════════════════════════════════════════════
    # WAVEFORM (заголовок и время наложены прямо на волну)
    # ══════════════════════════════════════════════════════════════
    ...
    # ══════════════════════════════════════════════════════════════
    # ПОДСВЕТКА АКТИВНОЙ КАРТОЧКИ (только рамка — без смены фона)
    # ══════════════════════════════════════════════════════════════
    ...
    # ══════════════════════════════════════════════════════════════
    # ОЧИСТКА ИСТОРИИ / ПУСТОЕ СОСТОЯНИЕ
    # ══════════════════════════════════════════════════════════════
    def clear_history():
    ...
    def _make_card(parent, entry):
        ...
        CompatCTkLabel(left, text=entry.get("date", ""), fg_color=Colors.BG_CARD,
                      text_color=Colors.ACCENT, font=("Segoe UI", scaled_font_size(8), "bold"),
        ...
        CompatCTkLabel(left, text=meta, fg_color=Colors.BG_CARD, text_color=Colors.TEXT_DIM,
                      font=("Segoe UI", scaled_font_size(8)), anchor="w").pack(fill="x", pady=(2, 4))
        text_preview = entry.get("text", "").replace("\n", " ")
        CompatCTkLabel(left, text=_truncate(text_preview, 90), fg_color=Colors.BG_CARD,
                      text_color=Colors.TEXT_MAIN, font=("Segoe UI", scaled_font_size(11)),
        ...
        btn_listen = _round_btn(actions, "▶", _play_entry, diameter=32, disabled=not audio_exists)
        ...
        btn_reuse = _round_btn(actions, "↩", _reuse, diameter=32)
    ...
    btn_clear = _round_btn(clear_row, "🗑", clear_history, diameter=34, danger=True)
    ...
    # — Мини-плеер: плавающая карточка снизу —
    ...
    btn_back10 = _round_btn(ctrl_row, "⏪", lambda: seek_rel(-10), diameter=36)
    btn_back10.pack(side="left", padx=4)
    btn_back5 = _round_btn(ctrl_row, "⏮", lambda: seek_rel(-5), diameter=36)
    btn_back5.pack(side="left", padx=4)

    play_btn = _round_btn(ctrl_row, "▶", toggle_play, diameter=54, primary=True)
    play_btn.pack(side="left", padx=10)

    btn_fwd5 = _round_btn(ctrl_row, "⏭", lambda: seek_rel(5), diameter=36)
    btn_fwd5.pack(side="left", padx=4)
    btn_fwd10 = _round_btn(ctrl_row, "⏩", lambda: seek_rel(10), diameter=36)
