...
def _create_placeholder_overlay(
    parent, text_widget, text: str, *, x=12, y=10, fg=None, bg=None, font=None
):
...
def _update_input_placeholder_text(text: str):
    ...
    if state._hint_text_var is not None:
        try:
            state._hint_text_var.set(
                "Enter — отправить · Ctrl+Enter — отправить без комментария · Shift+Enter — новая строка"
            )
        except Exception:
            pass
