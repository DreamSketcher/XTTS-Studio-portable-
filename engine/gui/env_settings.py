...
class EnvSettingsWindow(ctk.CTkToplevel):
    ...
    def __init__(self, master=None):
        ...
        footer_card = ctk.CTkFrame(footer_wrap, fg_color=bg_input, border_width=1, border_color=border_color, corner_radius=10)
        footer_card.pack(fill="x", padx=8, pady=6)
        
        # Статус-строка футера
        self.lbl_progress_status = ctk.CTkLabel(footer_card, text="", text_color=text_dim, font=ctk.CTkFont(size=11), justify="left", anchor="w")
        ...
        sec_deps = ctk.CTkFrame(outer_frame, fg_color=bg_input, border_width=1, border_color=border_color)
        sec_deps.pack(fill="x", pady=(0, 15), padx=5)
        
        lbl_deps_title = ctk.CTkLabel(sec_deps, text=t("section_dependencies"), font=ctk.CTkFont(weight="bold", size=14), text_color=text_main)
        lbl_deps_title.pack(anchor="w", padx=15, pady=(10, 5))
        
        self.lbl_deps_status = ctk.CTkLabel(
            sec_deps, text="🔍 Статус загружается...", text_color=text_dim, 
        ...
        def start_targeted_install(pkg_key, pip_spec):
            ...
            def _install_thread():
                ...
                self.after(0, lambda: self.lbl_progress_status.configure(text=f"🔧 Восстанавливаю {pkg_key}..."))
                ...
                try:
                    ...
                    if pkg_key == "torch":
                        env_setup.install_torch(progress_cb=progress_cb, resume=False, variant="cpu")
                    elif pkg_key == "rvc_python":
                        env_setup.install_rvc(progress_cb=progress_cb)
                    else:
                        cmd = [
                            env_setup.PYTHON_EXE, "-m", "pip", "install",
                            pip_spec,
                            "--target", env_setup.SITE_PACKAGES,
                            "--upgrade",
                            "--no-deps",
                            "--force-reinstall",
                            "--no-cache-dir"
                        ...
                        proc = subprocess.Popen(
                            cmd, cwd=env_setup.PROJECT_ROOT, env=env,
                    ...
                    def success_done():
                        ...
                        set_progress(100)
                        self.lbl_progress_status.configure(text=f"✅ Пакет '{pkg_key}' успешно восстановлен!")
                        set_status(t("status_ready"))
                        run_async_diagnostics(force_refresh=True) # Перескан статусов
                        mb.showinfo(t("update_done_title"), f"Пакет '{pkg_key}' успешно восстановлен!", parent=self)
                    ...
                    def failed_done(err_msg=str(e)):
                        ...
                        set_progress(0)
                        set_status("Ожидание...")
                        self.lbl_progress_status.configure(text=f"❌ Ошибка восстановления: {err_msg}")
                        mb.showerror(t("update_error_title"), f"Не удалось восстановить {pkg_key}:\n{err_msg}", parent=self)
                        _release_install_lock()
        ...
        for key, friendly_name in self.dependency_names.items():
            ...
            lbl_name = tk.Label(row_frame, text=friendly_name, font=("Segoe UI", scaled_font_size(9)), bg=bg_input, fg=text_main, width=32, anchor="w")
            lbl_name.pack(side="left", padx=5)
            
            lbl_stat = tk.Label(row_frame, text="⏳ Ожидание...", font=("Segoe UI", scaled_font_size(9), "bold"), bg=bg_input, fg=text_dim)
            ...
            btn_fix = CompatCTkButton(
                ...
                corner_radius=6,
                fg_color=btn_color, text_color=text_main, hover_color=bg_hover,
        ...
        self.btn_toggle_details = ctk.CTkButton(
            btn_row, text=t("btn_show_details"), command=toggle_details,
            fg_color=btn_color, hover_color=bg_hover, text_color=text_main,
            width=scaled_size(150, min_size=130), height=28
        )
        ...
        self.btn_refresh_deps = ctk.CTkButton(
            btn_row, text="🔍 Проверить статус", command=lambda: run_async_diagnostics(force_refresh=True),
            fg_color=bg_active, hover_color=bg_hover, text_color=text_main,
            width=scaled_size(150, min_size=130), height=28
        )
        ...
        def run_async_diagnostics(force_refresh=False):
            try:
                if not self.winfo_exists(): return
                self.lbl_deps_status.configure(text="🔍 Сканирую состояние библиотек...", text_color=text_dim)
            except Exception:
                pass
                    ...
                    try:
                        ...
                        if "error" in results:
                            if self.lbl_deps_status.winfo_exists():
                                self.lbl_deps_status.configure(text=f"❌ Сбой диагностики: {results['error']}", text_color="#b22222")
                            return
                        ...
                        if self.lbl_deps_status.winfo_exists():
                            if failed_count == 0:
                                self.lbl_deps_status.configure(text=t("status_all_healthy"), text_color="#2e7d32")
                            else:
                                self.lbl_deps_status.configure(text=f"{t('status_some_failed')} ({failed_count})", text_color="#b22222")
                    except Exception:
                        pass
        ...
        def run_diagnostics():
            ...
            status = env_setup.run_full_diagnostics(force_refresh=True) # Форсируем принудительный перескан по кнопке
            ...
            update_variant_label()
            run_async_diagnostics(force_refresh=True) # Обновляем раскрывающийся список зависимостей
        ...
        def disable_buttons():
            try:
                if not self.winfo_exists(): return
                if self.btn_gpu.winfo_exists(): self.btn_gpu.configure(state="disabled")
                if self.btn_cpu.winfo_exists(): self.btn_cpu.configure(state="disabled")
                if self.chk_auto.winfo_exists(): self.chk_auto.configure(state="disabled")
                if self.btn_check.winfo_exists(): self.btn_check.configure(state="disabled")
                if self.btn_clean.winfo_exists(): self.btn_clean.configure(state="disabled")
                if self.btn_diag.winfo_exists(): self.btn_diag.configure(state="disabled")
                if self.btn_recovery.winfo_exists(): self.btn_recovery.configure(state="disabled")
                if self.seg_pref.winfo_exists(): self.seg_pref.configure(state="disabled")
                if self.btn_toggle_details.winfo_exists(): self.btn_toggle_details.configure(state="disabled")
                if self.btn_refresh_deps.winfo_exists(): self.btn_refresh_deps.configure(state="disabled")
        ...
        def enable_buttons():
            try:
                ...
                if self.btn_cpu.winfo_exists(): self.btn_cpu.configure(state="normal")
                if self.chk_auto.winfo_exists(): self.chk_auto.configure(state="normal")
                if self.btn_check.winfo_exists(): self.btn_check.configure(state="normal")
                if self.btn_clean.winfo_exists(): self.btn_clean.configure(state="normal")
                if self.btn_diag.winfo_exists(): self.btn_diag.configure(state="normal")
                if self.btn_recovery.winfo_exists(): self.btn_recovery.configure(state="normal")
                if self.seg_pref.winfo_exists(): self.seg_pref.configure(state="normal")
                if self.btn_toggle_details.winfo_exists(): self.btn_toggle_details.configure(state="normal")
                if self.btn_refresh_deps.winfo_exists(): self.btn_refresh_deps.configure(state="normal")
