...
class ProsodyLayer:
    ...
    CONCLUSION_WORDS = {
        "поэтому", "итак", "таким образом", "следовательно",
        "therefore", "thus", "so", "hence", "consequently"
    }
    EMPHASIS_WORDS = {
        "важно", "главное", "обратите внимание", "заметьте",
        "important", "note that", "key point", "remember", "crucial"
    }
    EXAMPLE_WORDS = {
        "например", "к примеру", "допустим", "скажем",
    ...
    def _get_pause(self, pause_type: str) -> str:
        ...
        if intensity < 0.8:
            if base == self.PAUSE_LONG:   return self.PAUSE_MEDIUM
            if base == self.PAUSE_MEDIUM: return self.PAUSE_SHORT
        if intensity > 1.2:
            if base == self.PAUSE_SHORT:  return self.PAUSE_MEDIUM
            if base == self.PAUSE_MEDIUM: return self.PAUSE_LONG
        return base
...
def create_prosody_layer(mode="balanced", intensity=1.0, breath_length="medium"):
    return ProsodyLayer(
        ProsodyConfig(mode=mode, intensity=intensity, breath_length=breath_length)
    )